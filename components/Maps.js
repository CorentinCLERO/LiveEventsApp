...
import MapsFetch from "./MapsFetch";
...
const Maps = () => {
    ...
    useEffect(() => {
        let postsURL = "http://mspr2023.go.yj.fr/wp-json/locatoraid/v3/locations"
        axios.get(postsURL).then((res) => {
            setLocalisations(Object.values(JSON.parse(res.request._response)))
            console.log(Array.isArray(localisations))
        }).catch((error) => {
    ...
    useEffect(() => {
        ...
        /* if (localisations.length && typeof localisations[1].id === 'string') {
            setReceivedPosts(true);
            console.log("c'est " + receivedPosts)
        } */
    }, [localisations, receivedPosts]);
